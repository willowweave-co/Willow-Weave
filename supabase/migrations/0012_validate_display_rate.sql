-- ═══════════════════════════════════════════════════════════════════════════
-- Harden place_order against a bogus display_total (audit finding, low sev).
--
-- place_order is a security-definer RPC callable directly by the anon key, so
-- the server action's rate lookup (app/actions/checkout.ts) can be bypassed by
-- calling the RPC directly with any p_display_rate. That never affects the real
-- PKR `total` (computed server-side from DB prices), but a direct caller could
-- store a nonsense display_total (e.g. 0, negative, or absurdly large) that
-- then prints on the confirmation page / packing slip.
--
-- Fix: only derive display_total from a rate inside a REALISTIC band. A rate
-- can't be perfectly validated in-DB without a trusted rate source, but it can
-- be bounded so no grossly misleading figure is stored. Every supported display
-- currency is stronger than PKR, so a legitimate "units per 1 PKR" rate sits in
-- roughly [0.0027 (GBP) … 0.0135 (SAR)]; even the world's strongest currencies
-- stay near ~0.001. The band [0.0001, 100] clears all of those with wide margin
-- yet rejects the abusive extremes — a near-zero rate (which stored "$0" in the
-- audit) or an absurdly large one. Out-of-band → display_total NULL, which the
-- storefront renders as plain PKR. Legitimate orders are unaffected. The real
-- PKR `total` is never derived from this rate, so money is unaffected regardless.
--
-- CREATE OR REPLACE with the identical signature: preserves the existing
-- execute grant (no drop → no grant reset) and keeps this file re-runnable.
-- ═══════════════════════════════════════════════════════════════════════════
...
begin
  ...
  values (p_customer_name, p_phone, nullif(trim(coalesce(p_email,'')), ''), p_address, p_city,
          ...
          v_payment,
          v_currency,
          -- display_total is cosmetic; only derive it from a rate in a realistic
          -- band (see header) so a direct RPC caller can't store a misleading
          -- figure. Out-of-band rates → NULL, which the storefront shows as PKR.
          case
            when v_currency <> 'PKR' and p_display_rate is not null
                 and p_display_rate >= 0.0001 and p_display_rate <= 100
