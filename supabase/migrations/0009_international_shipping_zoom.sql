-- 1) Focal ZOOM for hero/banner crops (percent, 100 = no zoom).
--    Hero slides live in store_settings.hero_slides jsonb — no change needed.
alter table public.collections
  add column if not exists banner_focal_zoom int check (banner_focal_zoom between 100 and 300);

-- 2) International shipping: orders record a country, and the enabled
--    countries + per-country fees + checkout note live in settings jsonb:
--    { note, countries: [{ name, fee }] }. null = international off.
alter table public.orders
  add column if not exists country text not null default 'Pakistan';

alter table public.store_settings
  add column if not exists intl_shipping jsonb;

-- 3) place_order gains p_country. Drop the old 8-arg overload first — with
--    a defaulted 9th parameter both would match an 8-arg RPC call and
--    PostgREST would refuse the ambiguity.
