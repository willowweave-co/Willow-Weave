-- 1) Bank-transfer payment option + announcement bar colour.
--    bank_transfer jsonb: { bankName, accountName, accountNumber, iban };
--    null/empty = the option is hidden at checkout.
...
-- 2) place_order gains p_payment_method. Drop the 9-arg overload first
--    (defaulted params make mixed-arity RPC calls ambiguous otherwise).
